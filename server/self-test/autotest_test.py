...
from autotest_lib.server import autotest_bin
...
class AutotestTestCase(unittest.TestCase):
    def setUp(self):
        self.autotest = autotest_bin.Autotest()
    ...
    def testGetAutoDir(self):
        ...
        self.assertEqual('/stuff/autotest',
                         autotest_bin.Autotest.get_installed_autodir(host))
    ...
    def testInstallFromSVN(self):
        ...
        self.assertEqual(host.commands,
                         ['svn checkout '
                          + autotest_bin.AUTOTEST_SVN + ' ' + _TOP_PATH])
        ...
        class MockFirstInstallFailsHost:
            ...
            def run(self, command):
                if command == "grep autodir= /etc/autotest.conf":
                    ...
                    self.commands.append(command)
                    first = ('svn checkout ' + autotest_bin.AUTOTEST_SVN + ' ' +
                             _TOP_PATH)
                    if (command == first):
                        raise autotest_bin.AutoservRunError(
                                "svn not found")

        host = MockFirstInstallFailsHost()
        self.autotest.install(host)
        self.assertEqual(host.commands,
                         ['svn checkout ' + autotest_bin.AUTOTEST_SVN +
                          ' ' + _TOP_PATH,
                          'svn checkout ' + autotest_bin.AUTOTEST_HTTP +
                          ' ' + _TOP_PATH])
